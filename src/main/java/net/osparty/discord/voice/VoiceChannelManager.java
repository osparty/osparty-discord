package net.osparty.discord.voice;
...
/**
 * Level-1 Discord voice-channel provisioning via JDA. Ported verbatim from the API's old
 * {@code DiscordBotService}; it now lives behind this service's internal REST API so the gateway
 * connection is a singleton no matter how many API instances call it.
 *
 * <p>Each party gets one voice channel under a configured category, with {@code VIEW_CHANNEL} denied to
 * {@code @everyone} so the channel is unlisted and reachable only via the short-lived invite we mint and
 * hand back to the host.
 *
 * <p>Create and grant run synchronously ({@code complete()}): the caller (the API's WS handler thread) is
 * blocked on the reply, and JDA still honours the gateway rate limiter under the hood. Revoke, disconnect
 * and delete are fire-and-forget ({@code queue()}), so the controller returns immediately for those.
 */
@Service
public class VoiceChannelManager {
	private static final Logger log = LoggerFactory.getLogger(VoiceChannelManager.class);
	...
	public VoiceChannelManager(JDA jda,
		...
		@Value("${app.discord.invite-max-uses:0}") int inviteMaxUses) {
		this.jda = jda;
	...
	public Optional<VoiceChannelInfo> createForParty(PartyRef party, Collection<String> allowedDiscordIds) {
		Guild guild = jda.getGuildById(guildId);
		if (guild == null) {
			log.warn("Discord guild {} not found; cannot create channel for party {}", guildId, party.id());
			return Optional.empty();
		}
		try {
			ChannelAction<VoiceChannel> action = newChannelAction(guild, channelName(party))
			...
			if (allowedDiscordIds != null) {
				for (String discordId : allowedDiscordIds) {
					try {
						action = action.addMemberPermissionOverride(Long.parseLong(discordId), MEMBER_PERMS, null);
					}
			...
			VoiceChannel channel = action
				.reason("OSParty voice channel for party " + party.id())
				.complete();
			String inviteUrl = ((IInviteContainer) channel).createInvite()
				...
				.complete()
				.getUrl();
			log.info("Created Discord voice channel {} for party {}", channel.getId(), party.id());
			return Optional.of(new VoiceChannelInfo(channel.getId(), inviteUrl));
		}
		catch (Exception e) {
			log.warn("Failed to create Discord voice channel for party {}: {}", party.id(), e.toString());
			return Optional.empty();
		}
	}
	...
	public boolean grantAccess(String channelId, String discordId) {
		...
		try {
			...
			// Run synchronously (complete()) so we only report success once the override has actually landed on
			// Discord's side — the caller opens the invite the moment we reply, and if the grant were still in
			// flight the member would find the channel invisible on their first try. The member may not be
			// cached, so fetch them first.
	...
	public void disconnectFromChannel(String channelId, String discordId) {
		...
		try {
			...
			// With MemberCachePolicy.VOICE, a member currently in any voice channel is cached; null here means
			// they aren't connected to voice at all, so there's nothing to disconnect.
	...
	private ChannelAction<VoiceChannel> newChannelAction(Guild guild, String name) {
	...
	private static String channelName(PartyRef party) {
		List<String> parts = new ArrayList<>();
		if (party.host() != null && !party.host().isBlank()) {
			parts.add(party.host().trim());
		}
		if (party.inviteCode() != null && !party.inviteCode().isBlank()) {
			parts.add(party.inviteCode().trim());
		}
		if (party.activity() != null && !party.activity().isBlank()) {
			parts.add(party.activity().trim());
		}
		String name = parts.isEmpty() ? "party-" + party.id() : String.join(" | ", parts);
	...
	public record VoiceChannelInfo(String channelId, String inviteUrl) {
	}
}
